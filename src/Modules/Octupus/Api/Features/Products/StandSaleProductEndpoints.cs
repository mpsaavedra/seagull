...
{
    public static string ApiEndpoint = "/api/stand-sale-products/";
    public void MapEndpoints(WebApplication app)
    {
        app.MapGet(ApiEndpoint, (IMessageBus bus, int pageIndex = 1, int pageSize = 50, CancellationToken ct = default) =>
            Result
                ...
                .TryCatch(async qry =>
                {
                    var response = await bus.InvokeAsync<(List<StandSaleProductDto> Data, bool HasPreviousPage, bool HasNextPage)>(qry!, ct);
                    return Result.Success(PaginatedResponse<StandSaleProductDto>.CreatePaginated(
                        response.Data,
        ...
        app.MapGet(ApiEndpoint + "{id}", (IMessageBus bus, string id, CancellationToken ct = default) =>
            Result
                .Create("GetBydId")
                .Map(_ => new GetByIdStandSaleProduct(id))
                .TryCatch(async qry =>
                {
                    var response = await bus.InvokeAsync<StandSaleProductDetailsDto>(qry!, ct);
