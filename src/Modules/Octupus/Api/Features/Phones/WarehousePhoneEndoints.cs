    ...
    public void MapEndpoints(WebApplication app)
    {
        ...
        app.MapGet(ApiEndpoint, (IMessageBus bus, int pageIndex = 1, int pageSize = 50, CancellationToken ct = default) =>
            Result
                ...
                .TryCatch(async qry =>
                {
                    var response = await bus.InvokeAsync<(List<WarehousePhoneDto> Data, bool HasPreviousPage, bool HasNextPage)>(qry!, ct);
                    return Result.Success(PaginatedResponse<WarehousePhoneDto>.CreatePaginated(
                        response.Data,
